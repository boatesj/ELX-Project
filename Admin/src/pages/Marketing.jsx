  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <button onClick={onBack} className="inline-flex items-center gap-2 text-xs text-gray-400 hover:text-white transition">
          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7" />
          </svg>
          Back to pipeline
        ...
        <button onClick={() => onDelete(p._id)}
          className="inline-flex items-center gap-1.5 text-xs text-red-400 hover:text-red-300 border border-red-500/30 rounded-lg px-3 py-1.5 hover:bg-red-500/10 transition">
      ...
      {view === "pipeline" && (
        <div>
          ...
          {loading ? (
            ...
            <div className="space-y-2">
              {filtered.map((p) => (
                <div key={p._id} className="flex items-center gap-2 bg-[#020617] border border-[#1f2937] rounded-2xl px-4 py-3 hover:border-gray-600 transition group">

                  {/* Tick — mark next action done */}
                  <button
                    ...
                    onClick={async (e) => {
                      ...
                      const res = await fetch(`${MARKETING_API}/prospects/${p._id}`, {
                        ...
                        body: JSON.stringify({ nextActionDate: null, nextActionNote: "", note: "Action marked complete." }),
                      });
                      const data = await res.json();
                      if (res.ok) setProspects((prev) => prev.map((x) => x._id === p._id ? data.prospect : x));
                    }}
                    className="flex-shrink-0 w-7 h-7 rounded-full border border-[#1f2937] flex items-center justify-center text-gray-600 hover:border-emerald-500/50 hover:text-emerald-400 hover:bg-emerald-500/10 transition"
                  >
                  ...
                  <button onClick={() => setSelected(p)} className="flex-1 min-w-0 text-left">
                    <div className="flex items-center gap-2 flex-wrap">
                      <p className="font-semibold text-gray-100 truncate text-sm">{p.name}{p.company ? ` — ${p.company}` : ""}</p>
                      {stageChip(p.stage)}
                      {p.playbookDay > 0 && (
                    ...
                    <div className="flex items-center gap-3 mt-0.5 flex-wrap">
                      <p className="text-xs text-gray-500">{sectorLabel(p.sector)} · {p.channel}</p>
                      {p.nextActionDate && (
                      ...
                      {p.nextActionNote && <p className="text-xs text-gray-600 truncate max-w-[200px]">{p.nextActionNote}</p>}
                    </div>
                  </button>
                  ...
                  <button
                    ...
                    onClick={async (e) => {
                      ...
                      await fetch(`${MARKETING_API}/prospects/${p._id}`, { method: "DELETE", headers: authHeaders() });
                      setProspects((prev) => prev.filter((x) => x._id !== p._id));
                    }}
                    className="flex-shrink-0 w-7 h-7 rounded-full border border-[#1f2937] flex items-center justify-center text-gray-600 hover:border-red-500/50 hover:text-red-400 hover:bg-red-500/10 transition"
                  >
                    <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                    </svg>
