...
const renderStars = (rating = 5) => (
  <div className="flex items-center gap-1">
    {Array.from({ length: 5 }).map((_, index) => (
      <FaStar key={index} className={index < rating ? "text-[#FFA500]" : "text-gray-300"} />
    ))}
  </div>
);
      ...
      <div className="mx-auto w-full max-w-6xl px-4 md:px-6 lg:px-8">
        <div className="mb-10 flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
          <div className="max-w-3xl">
            <span className="inline-flex items-center rounded-full bg-[#1A2930] text-[#FFA500] px-3 py-1 text-[11px] md:text-xs font-semibold tracking-[0.16em] uppercase mb-3">Read our reviews</span>
            <h2 className="text-3xl md:text-[2.5rem] font-semibold tracking-tight text-[#111827] mb-3 uppercase">Real feedback from real shippers.</h2>
        ...
        <div className="mb-10 grid gap-6 md:grid-cols-3">
          {[
            { label: "Communication", title: "Clear updates at key milestones", body: "From booking and port delivery to vessel departure and arrival, we keep you informed so you&rsquo;re never guessing where your shipment is." },
            { label: "Focus", title: "UK–Africa routes, every day", body: "Our work is centred on UK exports into African ports, so we know the vessels, routes and paperwork your shipment depends on." },
        ...
        <div className="grid gap-6 md:grid-cols-3">
          {testimonials.map((story) => (
            <article key={story.id} className="flex flex-col rounded-2xl border border-gray-200 bg-white p-5 md:p-6 shadow-sm">
              ...
              <p className="text-sm md:text-[15px] text-gray-700 leading-relaxed mb-4">&ldquo;{story.quote}&rdquo;</p>
              <div className="mt-auto flex flex-col gap-2">
                ...
                <p className="text-[11px] md:text-xs text-gray-500 flex items-center gap-1.5">
                  <FaCheckCircle className="text-[#FFA500]" /><span>Verified Ellcworth shipper.</span>
                </p>
              </div>
            </article>
          ))}
