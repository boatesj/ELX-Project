...
import {
  FaEnvelope, FaPhone, FaBars, FaTimes, FaSignOutAlt,
  FaUserCircle, FaChevronDown, FaMapMarkedAlt,
} from "react-icons/fa";
import {
  readCustomerSession, clearCustomerAuth,
...
function Dropdown({ label, items, mapLink }: { label: string; items: DropdownGroup[] | DropdownItem[]; mapLink?: boolean }) {
  ...
  const isGrouped = (items as DropdownGroup[])[0]?.group !== undefined;

  return (
    <li className="relative" ref={ref}>
      <button type="button" onClick={() => setOpen((o) => !o)}
        className="flex items-center gap-1.5 hover:text-black/70 transition" aria-expanded={open}>
        {label}
      ...
      {open && (
        <div className="absolute left-0 top-full mt-2 z-50 min-w-[230px] rounded-2xl bg-[#0B141A] border border-white/10 shadow-[0_16px_48px_rgba(0,0,0,0.5)] overflow-hidden">
          <ul className="py-2">
            {isGrouped ? (
              (items as DropdownGroup[]).map((group) => (
                <li key={group.group}>
                  <p className="px-4 pt-3 pb-1 text-[10px] font-bold tracking-[0.2em] uppercase text-[#FFA500]/80">{group.group}</p>
                  <ul>
                    {group.items.map((item) => (
                      <li key={item.label}>
                        <Link href={item.to} onClick={() => setOpen(false)}
                          className="block px-4 py-2 text-xs font-semibold tracking-[0.12em] text-white/85 hover:text-white hover:bg-white/10 transition">
                          {item.label}
                        </Link>
                      </li>
                    ))}
              ...
              (items as DropdownItem[]).map((item) => (
                <li key={item.label}>
                  <Link href={item.to} onClick={() => setOpen(false)}
                    className="block px-4 py-2.5 text-xs font-semibold tracking-[0.12em] text-white/85 hover:text-white hover:bg-white/10 transition">
                    {item.label}
                  </Link>
                </li>
              ))
...
export default function Navbar() {
  ...
  const [customerAuth, setCustomerAuth] = useState<ReturnType<typeof readCustomerSession> | null>(null);

  const router   = useRouter();
  const pathname = usePathname();
  const isHome   = pathname === "/";

  useEffect(() => { setCustomerAuth(readCustomerSession()); }, []);
  ...
  const customerName = String(
    ...
    (customerAuth?.user as Record<string,unknown>)?.accountHolderName ||
      ...
      <div className="w-full bg-[#1A2930] text-white border-b border-white/10">
        <div className="container mx-auto flex justify-between items-center py-2.5 px-4 md:px-8 lg:px-16">
          <Link href="/" className="flex items-center gap-3 group">
            <img src="/Logo_w.svg" alt="Ellcworth Express logo" className="w-11 h-auto md:w-14 transition-transform duration-150 group-hover:scale-[1.02]" />
            <div className="flex flex-col leading-tight">
              ...
              <span className="text-[0.55rem] sm:text-[0.6rem] tracking-[0.25em] uppercase text-[#FFA500] mt-1">
      ...
      <div className="w-full bg-[#FFA500] hidden md:block border-b border-black/10">
        <div className="container mx-auto flex items-center justify-between py-3 px-4 md:px-8 lg:px-16">
          <ul className="flex gap-6 text-xs md:text-sm font-semibold text-[#1A2930] tracking-[0.12em] capitalize items-center">
            <li>
              <button type="button" onClick={() => goToSection("Header")} className="hover:text-black/70 transition">Home</button>
            </li>
          ...
          <div className="flex items-center gap-3">
            <button type="button" onClick={() => goToSection("quote")}
        ...
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          <div className="absolute top-0 right-0 h-full w-[88%] max-w-[380px] bg-[#0B1118] border-l border-white/10 shadow-[0_24px_80px_rgba(0,0,0,0.75)] flex flex-col">
            <div className="px-5 py-5 border-b border-white/10 bg-gradient-to-b from-[#0E1B20] to-[#0B1118]">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="text-white font-semibold tracking-[0.16em]">MENU <span className="text-[#FFA500]">→</span></p>
                  {isCustomerSignedIn && (
            ...
            <nav className="px-5 py-6 overflow-y-auto flex-1">
              ...
              <button type="button" onClick={() => goToSection("Header")}
                ...
                {servicesOpen && (
                  <div className="mt-1 ml-3 space-y-1">
                    {SERVICES_LINKS.map((group) => (
                      <div key={group.group}>
                        <p className="px-2 pt-2 pb-1 text-[10px] font-bold tracking-[0.2em] uppercase text-[#FFA500]/80">{group.group}</p>
                        {group.items.map((item) => (
                          <Link key={item.label} href={item.to} onClick={closeMenu}
                            className="block rounded-xl border border-white/10 bg-white/5 px-4 py-2.5 text-xs font-semibold tracking-[0.12em] uppercase text-white/80 hover:bg-white/10 transition mb-1">
                            {item.label} →
                          </Link>
                        ))}
                      </div>
                ...
                {destOpen && (
                  <div className="mt-1 ml-3 space-y-1">
                    {DESTINATION_LINKS.map((item) => (
                      <Link key={item.label} href={item.to} onClick={closeMenu}
                        className="block rounded-xl border border-white/10 bg-white/5 px-4 py-2.5 text-xs font-semibold tracking-[0.12em] uppercase text-white/80 hover:bg-white/10 transition">
                        {item.label} →
                      </Link>
                    ))}
              ...
              <div className="mt-4 pt-4 border-t border-white/10 space-y-3">
                {isCustomerSignedIn ? (
                  <>
                    <Link href="/myshipments" onClick={closeMenu}>
