...
export { PORTS, ORIGIN_COLOUR, DESTINATION_COLOUR };

const MapEmbed = ({ height = "500px", zoom = 2.4, center = [-10, 25] }) => {
  ...
  useEffect(() => {
    ...
    map.current.on("load", () => {
      ...
      const routeFeatures = origins.flatMap((o) =>
        destinations.map((d) => ({
          type: "Feature",
          geometry: { type: "LineString", coordinates: [o.coords, d.coords] },
      ...
      PORTS.forEach((port) => {
        ...
        new mapboxgl.Marker(el).setLngLat(port.coords).setPopup(popup).addTo(map.current);
      });
    });
