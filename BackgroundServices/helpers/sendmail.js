...
function requireEnv(env, keys) {
  ...
  if (missing.length) {
    ...
    const detail = [
      ...
      `Tip: if using .env, restart BackgroundServices after edits.`,
    ].join(" | ");
...
const dispatchMail = async ({ to, subject, html, text, from, replyTo }) => {
  const env = process.env;

  const resolvedFrom = String(from || pickFromAddress(env) || "").trim();
  if (!resolvedFrom) {
    throw new Error(
      "Email FROM address missing. Set EMAIL_FROM in BackgroundServices env.",
    );
  }
